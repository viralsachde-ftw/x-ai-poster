...
let cachedQueryId = 'IID9x6WsdMnTlXnzXGq8ng';
...
async function resolveQueryId() {
  try {
    const res = await fetch(OPENAPI_URL, { signal: AbortSignal.timeout(8000) });
    if (!res.ok) return null;
    const data = await res.json();
    const op = data.CreateTweet;
    if (op && op.queryId) {
      console.log('resolved CreateTweet queryId from github:', op.queryId);
      return op.queryId;
    }
  } catch (e) {
  ...
  try {
    ...
    const scriptUrls = [];
    const scriptRe = /(?:src|href)=["'](https:\/\/abs\.twimg\.com\/responsive-web\/client-web[^"']+\.js)["']/g;
    let m;
    ...
    for (const url of scriptUrls.slice(0, 10)) {
      try {
        const bundleRes = await fetch(url, { signal: AbortSignal.timeout(8000) });
        const bundle = await bundleRes.text();
        const opRe = /queryId:\s*"([A-Za-z0-9_-]+)"[^}]{0,200}operationName:\s*"CreateTweet"/;
        const match = bundle.match(opRe);
        if (match) {
          console.log('resolved CreateTweet queryId from JS bundle:', match[1]);
          return match[1];
        }
      } catch {
        continue;
    ...
    console.log('JS bundle scan failed:', e.message);
  }
...
async function postTweet(text) {
  ...
  async function tryPost(queryId) {
  ...
  let res = await tryPost(cachedQueryId);

  if (res.status === 404) {
    console.log('queryId expired (404), resolving fresh one...');
    const freshId = await resolveQueryId();
    if (freshId && freshId !== cachedQueryId) {
      cachedQueryId = freshId;
      res = await tryPost(cachedQueryId);
    }
  }
